#Menu pero con interfaz grafica
...
class CreateClientWindow(Toplevel, CenterWidgetMixin):
    def __init__(self, parent): 
        ...
        self.grab_set() #Nos obligan a interactuar primero con la ventana emergente, antes de poder interactuar con la principal
    ...
    def validate(self, event, index): 
        valor = event.widget.get() #Widget toma cualquier valor, en este caso, de ENTRY
        """
        if index == 0: 
        ...
        valido = helpers.dni_valido(valor, db.Clientes.lista) if index == 0 \
            else (valor.isalpha() and len(valor) >= 2 and len(valor) <= 30) #Esa contrabarra permite continuar el comando

        event.widget.configure({ "bg":"Green" if valido else "Red"}) 

        #Cambiar el estado del boton segun las validaciones 
        self.validaciones[index] = valido
        self.crear.config(state=NORMAL if self.validaciones == [1, 1, 1] else DISABLED) #Vuelve a activar el boton crear
...
class EditClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def validate(self, event, index): 
        ...
        valido = (valor.isalpha() and len(valor) >= 2 and len(valor) <= 30) #Esa contrabarra permite continuar el comando

        event.widget.configure({ "bg":"Green" if valido else "Red"}) 

        #Cambiar el estado del boton segun las validaciones 
        self.validaciones[index] = valido
        self.actualizar.config(state=NORMAL if self.validaciones == [1, 1] else DISABLED) #Vuelve a activar el boton crear
...
class MainWindow(Tk, CenterWidgetMixin): 
    ...
    def build(self): 
        ...
        treeview["columns"] = ("DNI", "Nombre", "Apellido") #Cantidad de columnas
    ...
    def delete(self): 
        ...
        if cliente:
            campos = self.treeview.item(cliente, "values") #Esto extraera los valores del cliente en el treeview
            confirmar = askokcancel(
                title="Confirmar borrado",
                message=f"¿Borrar el {campos[1]} {campos[2]}?",
                icon=WARNING)
            if confirmar: 
    ...
    def edit(self): 
        if self.treeview.focus(): #Chequeo tener un item seleccionado para modificarlo, en caso contrario, no aparecera nada
